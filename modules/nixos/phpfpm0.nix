...
let
  cfg = config.services.phpfpm0;

  runtimeDir = "/run/phpfpm0";
  ...
  fpmCfgFile = pool: poolOpts: pkgs.writeText "phpfpm0-${pool}.conf" ''
    [global]
...
in
{
  ...
  config = mkIf (cfg.pools != { }) {
    ...
    systemd.slices.phpfpm0 = {
      description = "PHP FastCGI Process manager pools slice";
    };

    systemd.targets.phpfpm0 = {
    ...
    systemd.services = mapAttrs'
      (pool: poolOpts:
        nameValuePair "phpfpm0-${pool}" {
          description = "PHP FastCGI Process Manager service for pool ${pool}";
          after = [ "network.target" ];
          wantedBy = [ "phpfpm0.target" ];
          partOf = [ "phpfpm0.target" ];
          serviceConfig =
            let
            ...
            {
              Slice = "phpfpm0.slice";
              # Below: 4 settings no compatible with execution inside container
              # (w/o SYS_ADMIN capabilities as   Docker )
              # 
              #PrivateDevices = true;
              #PrivateTmp = true;
              #ProtectSystem = "full";
